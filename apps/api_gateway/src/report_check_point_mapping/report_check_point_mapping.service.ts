  ...
  : Promise<ResponseDto> {
    try {
      ...
      const sqlQuery = `
        ...
        FROM
          "39131F99F8F44FB4A0F0F6D759497FF7"."PCF_DB_REPORT_CHECKPOINT_MAPPING" AS mapping
        LEFT JOIN
          "39131F99F8F44FB4A0F0F6D759497FF7"."PCF_DB_REPORT_MASTER" AS report ON mapping.REPORT_ID = report.ID
        LEFT JOIN
          "39131F99F8F44FB4A0F0F6D759497FF7"."PCF_DB_CHECK_POINT_MASTER" AS checkpoint ON mapping.CHECK_POINT_ID = checkpoint.ID
        LEFT JOIN
          "39131F99F8F44FB4A0F0F6D759497FF7"."PCF_DB_CUSTOMER_MASTER" AS customer ON mapping.CUSTOMER_ID = customer.ID
        WHERE
          mapping.IS_ACTIVE = 'Y'
